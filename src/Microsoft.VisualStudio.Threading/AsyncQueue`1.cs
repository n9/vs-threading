...
namespace Microsoft.VisualStudio.Threading
{
    ...
    public class AsyncQueue<T> : ThreadingTools.ICancellationNotification
    {
        ...
        public Task Completion
        {
            get
            {
                if (this.completedSource is null)
                {
                    lock (this.SyncRoot)
                    {
                        if (this.completedSource is null)
        ...
        public bool TryPeek([MaybeNullWhen(false)] out T value)
        {
            lock (this.SyncRoot)
            {
                if (this.queueElements is object && this.queueElements.Count > 0)
                {
...
#pragma warning restore CS8717 // A member returning a [MaybeNull] value introduces a null value for a type parameter.
        ...
        public Task<T> DequeueAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            lock (this.SyncRoot)
            {
                ...
                {
                    if (this.dequeuingWaiters is null)
                    {
...
#pragma warning restore CS8717 // A member returning a [MaybeNull] value introduces a null value for a type parameter.
        ...
        private bool TryDequeueInternal(Predicate<T>? valueCheck, [MaybeNullWhen(false)] out T value)
        {
            ...
            lock (this.SyncRoot)
            {
                if (this.queueElements is object && this.queueElements.Count > 0 && (valueCheck is null || valueCheck(this.queueElements.Peek())))
                {
        ...
        private void CompleteIfNecessary()
        {
            ...
            lock (this.SyncRoot)
            {
                transitionTaskSource = this.completeSignaled && (this.queueElements is null || this.queueElements.Count == 0);
